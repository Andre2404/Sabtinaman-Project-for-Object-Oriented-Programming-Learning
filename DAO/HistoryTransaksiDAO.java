...
public class HistoryTransaksiDAO {
    ...
    public List<HistoryTransaksi> getAllData(int userId, LocalDate startDate, LocalDate endDate) throws SQLException {
    List<HistoryTransaksi> list = new ArrayList<>();
    list.addAll(getTopUpData(userId, startDate, endDate));  // Panggil dengan filter tanggal
    list.addAll(getSewaAlatData(userId, startDate, endDate)); // Panggil dengan filter tanggal
    list.addAll(getPupukData(userId, startDate, endDate));   // Panggil dengan filter tanggal
    return list;
}


    public List<HistoryTransaksi> getFilteredData(int userId, boolean topUp, boolean sewa, boolean pupuk, LocalDate startDate, LocalDate endDate) throws SQLException {
    List<HistoryTransaksi> list = new ArrayList<>();
    if (topUp) list.addAll(getTopUpData(userId, startDate, endDate));
    if (sewa) list.addAll(getSewaAlatData(userId, startDate, endDate));
    if (pupuk) list.addAll(getPupukData(userId, startDate, endDate));
    return list;
}

// Data Top Up Saldo dengan filter tanggal
private List<HistoryTransaksi> getTopUpData(int userId, LocalDate startDate, LocalDate endDate) throws SQLException {
    String query = """
    ...
    """;
    return executeQueryWithDateRange(query, userId, startDate, endDate);
}

// Data Sewa Alat dengan filter tanggal
private List<HistoryTransaksi> getSewaAlatData(int userId, LocalDate startDate, LocalDate endDate) throws SQLException {
    String query = """
...
// Data Beli Pupuk dengan filter tanggal
private List<HistoryTransaksi> getPupukData(int userId, LocalDate startDate, LocalDate endDate) throws SQLException {
    String query = """
...
private List<HistoryTransaksi> executeQueryWithDateRange(String query, int userId, LocalDate startDate, LocalDate endDate) throws SQLException {
    List<HistoryTransaksi> list = new ArrayList<>();
    System.out.println("Executing query with date range: " + query + " | User ID: " + userId + " | From: " + startDate + " To: " + endDate);
    try (PreparedStatement stmt = connection.prepareStatement(query)) {
        stmt.setInt(1, userId);
        stmt.setDate(2, java.sql.Date.valueOf(startDate)); // Mengubah LocalDate menjadi java.sql.Date
        stmt.setDate(3, java.sql.Date.valueOf(endDate));   // Mengubah LocalDate menjadi java.sql.Date
...
//    """;
//    return executeQueryWithDateRange(query, companyId);
//}
//
